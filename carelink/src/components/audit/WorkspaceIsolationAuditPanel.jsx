...
export function WorkspaceIsolationAuditPanel({ result }) {
  if (!result) return null;
  const ok = (result.blockers || []).length === 0;
  return (
    <Card variant="default">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        <div className="card-title">Workspace Isolation</div>
        <span style={{ fontSize: 12, fontWeight: 700, color: ok ? '#22c55e' : '#ef4444' }}>
          {ok ? '✓ PASS' : '⛔ FAIL'} — {result.score}/100
        </span>
      </div>
      <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 8 }}>
        Workspaces: {result.details?.workspaceCount || 0} · Link by ID only · No source record mutation.
      </div>
...
export default WorkspaceIsolationAuditPanel;
