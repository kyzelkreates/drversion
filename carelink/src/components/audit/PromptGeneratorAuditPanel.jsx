...
export function PromptGeneratorAuditPanel({ result }) {
  if (!result) return null;
  const ok = (result.blockers || []).length === 0;
  return (
    <Card variant="default">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        <div className="card-title">Prompt Generator</div>
        <span style={{ fontSize: 12, fontWeight: 700, color: ok ? '#22c55e' : '#ef4444' }}>
          {ok ? '✓ PASS' : '⛔ FAIL'} — {result.score}/100
        </span>
      </div>
      <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 8 }}>
        Prompts: {result.details?.promptCount || 0} · Manual copy-paste only — no auto-execution.
      </div>
...
export default PromptGeneratorAuditPanel;
